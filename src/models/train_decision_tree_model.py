...
def predict_decision_tree_regression(
    model: DecisionTreeRegressor,
    df: pd.DataFrame,
    feature_cols: List[str],
) -> pd.Series:
    """Generate predictions from a trained decision tree model."""
    missing_features = [col for col in feature_cols if col not in df.columns]
    if missing_features:
        raise ValueError(f"Missing feature columns: {missing_features}")
...
def evaluate_decision_tree_predictions(
    df: pd.DataFrame,
    date_col: str,
    target_col: str,
...
def run_decision_tree_validation(
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    feature_cols: List[str],
    target_col: str,
    ...
    top_n: int = 10,
    bottom_n: int = 10,
) -> Tuple[DecisionTreeRegressor, pd.DataFrame, Dict[str, float]]:
    """
    Train decision tree model, predict on validation set, and evaluate.
    """
    _validate_feature_target_columns(train_df, feature_cols, target_col)
    _validate_feature_target_columns(val_df, feature_cols, target_col)

    model = train_decision_tree_regression(
        train_df=train_df,
    ...
    scored_val["prediction"] = predict_decision_tree_regression(model, scored_val, feature_cols)

    metrics = evaluate_decision_tree_predictions(
        df=scored_val,
        date_col=date_col,
        target_col=target_col,
